...
export class CourseListComponent {
  ...
  editItem(item:any){    
   ...
   //set detail id
   detailItem(item:any){    
    this.router.navigate([environment.courseDetails+"/"+item.id]);
  }
  ...
  onScroll(){
    setTimeout(()=>{
      ...
      this.no=this.no+1;
      const apiUrl = `${baseURL + 'AdminItems/CourseList?no='}${this.no}&cId=${this.catId}&sId=${this.subcatId}&pId=${this.pracatId}`;
  ...
  private itemlist() {
    ...
    this.itemslistloading = true;
    let n = 0;
    const apiUrl = `${baseURL + 'AdminItems/CourseList?no='}${n}&cId=${this.catId}&sId=${this.subcatId}&pId=${this.pracatId}`;
    this.commenservice.commenDataList(apiUrl).subscribe({
      next: (data) => {
